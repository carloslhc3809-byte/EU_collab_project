...
def filter_links(link_list, threshold_eff, hide_observed):
    if hide_observed:
        link_list = [l for l in link_list if l["status"] == "Predicted"]
...
with st.sidebar:
    ...
    hide_observed = st.checkbox("Hide observed links", value=True)
...
for node_id in df["node_id"]:
    ...
    filtered_node_links = filter_links(node_links, threshold_eff, hide_observed)
...
all_links_filtered = filter_links(all_links, threshold_eff, hide_observed)
